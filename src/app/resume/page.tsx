...
export default function ResumePage() {
  ...
  return (
        <div className="mx-auto max-w-3xl px-4 py-12">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">이력서 분석</h1>h1>
              <p className="text-gray-500 mb-8">
                      이력서를 업로드하면 AI가 면접 질문 포인트를 미리 분석해드립니다
              </p>p>
        
          {/* Upload area - label for reliable mobile file picker */}
              <label
                        htmlFor="file-input"
                        onDragOver={(e) => {
                                    e.preventDefault();
                        ...
                        onDrop={handleDrop}
                        className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-colors cursor-pointer block ${
                                    dragOver
                ...
                {file ? (
                                  <div className="flex flex-col items-center gap-3">
                                              ...
                                              <p className="font-semibold text-gray-900">{file.name}</p>p>
                                  ...
                                  <div className="flex flex-col items-center gap-3">
                                              ...
                                              <p className="text-sm text-gray-400">PDF, DOCX (최대 10MB)</p>p>
                                  </div>div>
                      )}
              </label>label>
          ...
          {error && (
                  <div className="mt-4 p-4 bg-red-50 rounded-xl border border-red-200">
                            <p className="text-sm text-red-700">{error}</p>p>
                  </div>div>
              )}
                  ...
                  <div className="mt-8 space-y-6">
                            <div className="p-6 bg-indigo-50 rounded-2xl">
                                        <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                                                      <span className="text-xl">🔍</span>span> 분석 결과 요약
                                        </h3>h3>
                                        <div className="space-y-3 text-sm">
                                                      <div>
                                                                      <span className="font-medium text-gray-700">이름:</span>span>{" "}
                                                                      <span className="text-gray-900">{resumeData.name}</span>span>
                                                      </div>div>
                                          {resumeData.experience.length > 0 && (
                                    <div>
                                                      <span className="font-medium text-gray-700">경력:</span>span>
                                    ...
                                    <div>
                                                      <span className="font-medium text-gray-700">기술 스택:</span>span>
                    ...
                    {resumeData.projects.length > 0 && (
                                <div className="p-6 bg-blue-50 rounded-2xl">
                                              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                                                              <span className="text-xl">💼</span>span> 주요 프로젝트
                                              </h3>h3>
                    ...
                    {resumeData.weakPoints.length > 0 && (
                                <div className="p-6 bg-amber-50 rounded-2xl">
                                              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                                                              <span className="text-xl">⚠️</span>span> 면접 질문 공격 포인트
                                              </h3>h3>
